...
# rm(modele_full, modele_sel)
rm(list = ls(pattern = "^m0."))
...
tendances_stoc <- tendances_stoc %>% 
  ...
  mutate(tendances_stoc_simplifie = case_when(tendances_stoc %in% c("Augmentation forte","Augmentation modérée",
                                                                    "Augmentation modérée à forte") ~ "Augmentation",
                                              tendances_stoc %in% c("Déclin fort","Déclin modéré",
                                                                    "Déclin modéré à fort") ~ "Déclin",
                                              TRUE ~ tendances_stoc),
...
AIC_dt_trait <- as.data.frame(AIC(m_n1_base, 
                                  ...
                                  m_n1_trait_all,
                                  m_n1_trait_allint, m_n1_trait_allint_1, m_n1_trait_allint_2))
...
#         stop(sprintf("La variable '%s' n'est pas dans le modèle.", x_var))
#       }
...
#   )
# 
#   # Créer un graphique par variable
...
plot_clmm_predictions_multi_x <- function(
    ...
    function(model, x_var) {
      ...
      emm_summary %>%
        mutate(
          ...
          lower = plogis(asymp.LCL),
          upper = plogis(asymp.UCL),
          # Logique pour la significativité (exemple : si l'intervalle de confiance contient 0)
  ...
  plots <- split(emm_all, emm_all$variable) %>%
    imap(~ {
      # Déterminer si toutes les interactions pour ce niveau sont non significatives
      ...
      p <- ggplot(.x, aes(
        ...
        # Lignes continues ou pointillées (selon la significativité globale)
        geom_line(
...
plot_clmm_predictions_multi_x(
  models = models,
  x_values = x_values,
  subtitle = "Effet de la tendance SHOC selon différentes variables (niveau 1)"
)
...
m_n1_trait_participant_allint_6 <- clmm(avis_niv1 ~ tendance_shoc_niv1 + 
                    abondance_log + regime + popularite + detectabilite + migrateur + gregaire + fauneFrance + pourcentage_stoc + 
                      sexe + anciennete + expert +
                    tendance_shoc_niv1:(sexe + anciennete + expert + fauneFrance + pourcentage_stoc) + (1 | email), data = dt1, link = "logit", Hess = TRUE)
...
AIC_dt_all_n1 <- as.data.frame(AIC(m_n1_base,
                                   ...
                                   m_n1_trait_participant_allint_4, m_n1_trait_participant_allint_5, m_n1_trait_participant_allint_6,
...
Variable réponse : avis_niv1
→ variable ordinale à 3 niveaux (Déclin < Stable < Augmentation).

Modèle : Cumulative Link Mixed Model
→ on modélise la probabilité cumulée d’être dans une catégorie plus élevée.

Effet aléatoire : (1 | email)
→ chaque répondant a une propension personnelle à répondre plus pessimiste/optimiste
→ variance = 0.45, non négligeable → bon choix de modèle mixte.
...
👉 Plus une espèce est abondante, grégaire ou bien connue, plus l’avis est sévère
→ biais classique de baseline shifting / exigence accrue
...
Les avis ne dépendent pas seulement de la tendance observée (SHOC), mais de la cohérence perçue entre cette tendance et les caractéristiques écologiques de l’espèce.
Une dégradation est jugée d’autant plus sévèrement qu’elle concerne une espèce abondante, populaire, insectivore ou migratrice, suggérant un raisonnement fondé sur la plausibilité écologique et la gravité implicite du signal.
...
# m_n2_2int <- clmm(avis_niv2 ~ tendance_shoc_niv2*abondance_log + (1 | email) + (1 | departement), data = dt2, link = "logit", Hess = TRUE)
# anova(m_n2_2, m_n2_2int)
...
# fauneFrance *** int ***
m_n2_trait_7 <- clmm(avis_niv2 ~ tendance_shoc_niv2 + fauneFrance + (1 | email), data = dt2, link = "logit", Hess = TRUE)
anova(m_n2_trait_7, m_n2_base)
m_n2_trait_7int <- clmm(avis_niv2 ~ tendance_shoc_niv2*fauneFrance + (1 | email), data = dt2, link = "logit", Hess = TRUE)
anova(m_n2_trait_7, m_n2_trait_7int)

# pourcentage_stoc *** int ***
m_n2_trait_8 <- clmm(avis_niv2 ~ tendance_shoc_niv2 + pourcentage_stoc + (1 | email), data = dt2, link = "logit", Hess = TRUE)
anova(m_n2_trait_8, m_n2_base)
m_n2_trait_8int <- clmm(avis_niv2 ~ tendance_shoc_niv2*pourcentage_stoc + (1 | email), data = dt2, link = "logit", Hess = TRUE)
...
plot_clmm_predictions_multi_x(
  models = models,
  x_values = x_values,
  subtitle = "Effet de la tendance SHOC selon différentes variables (niveau 2)"
)
...
AIC_dt_trait_niv_2 <- as.data.frame(AIC(
  m_n2_base,
                                ...
                                m_n2_2, m_n2_3, m_n2_4, m_n2_5, m_n2_6,
                                m_n2_3int, m_n2_4int, m_n2_5int, m_n2_6int,
                                m_n2_all, 
                                # m_n2_allint, 
                                m_n2_allint_2, m_n2_allint_3,
